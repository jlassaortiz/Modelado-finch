...
def ecuaciones(v, dv):
    i1,i2 = v
    dv[0]= i2
    dv[1]= - 1/(Ch*Lg)*i1 - (Rd/Lg)*i2 + (1/Lg)*destimulodt
...
Ch = calc_c(11000) # / 1.0 # Capacitancia Helmholtz
Rd = 1500 # Resistencia disipación 
...
if guardar:
    write(f'OUT_{version}_C_{Ch}_R_{Rd}_Lg_{Lg}_Ltraquea_{L}_coefReflex_{coef_reflexion}.wav', int(sampling_freq), out_scaled)
    write(f'IN_{version}_C_{Ch}_R_{Rd}_Lg_{Lg}_Ltraquea_{L}_coefReflex_{coef_reflexion}.wav', int(sampling_freq), in_scaled)
...
fig.suptitle(f'IN-OUT_C_{Ch}_R_{Rd}_Lg_{Lg}_Ltraquea_{L}_coefReflex_{coef_reflexion}')
plt.show()
...
fig.suptitle(f'FFT-IN-OUT_C_{Ch}_R_{Rd}_Lg_{Lg}_Ltraquea_{L}_coefReflex_{coef_reflexion}')
...
fig.suptitle(f'FFT_SMOOTH-IN-OUT_C_{Ch}_R_{Rd}_Lg_{Lg}_Ltraquea_{L}_coefReflex_{coef_reflexion}')

...
for lazo in range(1):
    ...
    # z son los coeficientes del polinomio
    # generamos objeto p, que es un polinomio de grado 5 (es z)
    p = np.poly1d(z)
    
    for i in range(np.int(tiempo_total/(dt))):
        # si se aplico la funcion recta, senito, exp se cumple:
        if(alpha[i]<0):
    ...
    write('sintesis_finch_2.wav'.format(lazo), 44100, scaled)
